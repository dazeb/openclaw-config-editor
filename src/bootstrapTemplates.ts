...
# Secret scan current repo (install via: brew install gitleaks)
...
# Recent authentication failures (macOS)
if [[ "$OSTYPE" == "darwin"* ]]; then
  log show --predicate 'subsystem == "com.openssh.sshd"' --last 1h | grep "Failed"
else
...
/usr/libexec/PlistBuddy -c "Print" /System/Library/CoreServices/SystemVersion.plist
...
2. Use Gatekeeper for app verification
3. Keep macOS and Xcode updated
4. Use Keychain for credential storage
5. Enable firewall in System Preferences
`,
    },
  },
  {
...
# Initialize test file (macOS compatible)
...
mkdir -p ~/Developer/projects/{project-name}/{src,tests,docs}

# Set Finder-friendly file attributes (optional)
if command -v SetFile &> /dev/null; then
  SetFile -a "E" ~/Developer/projects/project-name  # Exclude from Time Machine
fi
...
### README Template
- Project description
- Installation instructions (with macOS-specific steps)
- Usage examples
- API documentation
...
### macOS Documentation
- Use ~/Developer/ instead of /opt/ for projects
- Include Homebrew installation commands
- Mention macOS-specific tools (SetFile, mdls, etc.)
- Reference Xcode Command Line Tools when relevant
`,
    },
  },
  {
...
# Detect macOS and adjust paths
if [[ "$OSTYPE" == "darwin"* ]]; then
  echo "🍎 Detected macOS - using native paths"
  # Expand ~ to full home directory path
  WORKSPACE="${workspacePath}"
  WORKSPACE_EXPANDED="${workspacePath}"  
else
  echo "🐧 Detected Linux/other - using standard paths"
  WORKSPACE="${workspacePath}"
  WORKSPACE_EXPANDED="${workspacePath}"
fi

echo "Setting up agent workspace: $WORKSPACE_EXPANDED"

# Create workspace directory
mkdir -p "$WORKSPACE_EXPANDED"
cd "$WORKSPACE_EXPANDED"
...
# Set proper permissions on macOS (optional)
if [[ "$OSTYPE" == "darwin"* ]]; then
  ...
  chmod 644 *.md
  chmod 600 *SOUL.md *IDENTITY.md 2>/dev/null || true
fi
...
echo "📁 Location: $WORKSPACE_EXPANDED"
echo "📄 Files created:"
...
if [[ "$OSTYPE" == "darwin"* ]]; then
  ...
  echo "2. Delete BOOTSTRAP.md after reading it"
  echo "3. Start your agent with OpenClaw"
  echo "4. Consider backing up your workspace directory"
fi
`;
}
